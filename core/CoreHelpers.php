...
function cached_setting_item($item, $default = '', $isArray = false)
{
    ...
    $res = Setting::cachedItem($item, $default);
...
function setting_item_with_lang($item, $locale = '', $default = '', $withOrigin = true)
{
    ...
    return Setting::item($item . ($locale ? '_' . $locale : ''), ($locale and $withOrigin) ? setting_item($item, $default) : $default);
...
function setting_update_item($item, $val)
{
    ...
    $s = Setting::where('name', $item)->first();
    if (empty($s)) {
        $s = new Settings();
        $s->name = $item;
    }
