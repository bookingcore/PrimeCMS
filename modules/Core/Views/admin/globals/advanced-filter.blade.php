<div>
    ...
    <div class="mb-0">
        <label
            class="d-block"
            for="exampleInputEmail1">{{ __("Featured") }}</label>
        <select
            ...
            <option
                value="1"
                @if(Request()->is_featured == 1) selected @endif>{{ __("Only Featured") }}</option>
        </select>
    </div>
...
<script>
    $('.dungdt-select2-field').each(function() {
        $(this).on('change', () => {
            $wire.$set($(this).attr('name'), $(this).val());
        });
    });
</script>
@endscript
